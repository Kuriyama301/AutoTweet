...
**AutoTweet**は、スクレイピング技術を活用してXでのエンゲージメントを効率化するツールです。リプライ対象のポストを自動で選定し、テンプレートベースのリプライ文面案を提示。人間の最終判断を挟むことで、品質を保ちながら効率的にインプレッションを向上させます。
...
1. **ポスト検索・選定**（スクレイピング）
   ...
   - Playwrightで検索結果を取得（本文、作成者、プロフィール、URL）
   - 簡易ロジックで経営者のポストと思われるものを10件選定
     - プロフィールキーワード: 「代表」「CEO」「創業者」「経営」等
...
### 将来の拡張（本番化時）
- **AI生成リプライへの切り替え**（任意）
  - Grok API統合（$5課金で$150分クレジット）
  - OpenAI API統合（GPT-3.5-turbo: $0.5/1M入力）
  - テンプレート → AI生成文面に切り替え
- **X API移行**（$100/月）
  - 規約遵守、安定性向上
  - メインアカウント保護
